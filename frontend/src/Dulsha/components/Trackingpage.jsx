...
const Trackingpage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/Api/Addcollectingdata/last/collectingdetail`);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/get/data-location`);
        if (response.data && response.data.length > 0) {
          const { latitude, longitude } = response.data[0];
          setMarker({ lat: latitude, lng: longitude });
        } else {
